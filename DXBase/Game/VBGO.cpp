...
// Compile the  shader
//--------------------------------------------------------------------------------------
HRESULT VBGO::CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
void VBGO::Init(ID3D11Device* _GD)
{
	...
	hr = VBGO::CompileShaderFromFile(L"../Assets/shader.fx", "PS", "ps_4_0_level_9_1", &pPixelShaderBuffer);
...
void VBGO::SetWireframe(bool _bool, ID3D11Device* _GD)
{
	D3D11_RASTERIZER_DESC rasterDesc;
	if (_bool)
	{
	...
	// Create the rasterizer state from the description we just filled out.
	_GD->CreateRasterizerState(&rasterDesc, &m_pRasterState);
}
